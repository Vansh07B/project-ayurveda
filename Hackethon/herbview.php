...
<body>
    ...
    <div class="container my-5">
        ...
        <!-- Updated How to Use & Recipes Section -->
        <div class="my-5">
            <h3 class="section-title">🍵 How to Use & Recipes</h3>
            <ul>
                <?php
                    // Fetch recipes related to this herb using herb_id
                    $sql = "SELECT id, title FROM recipedb WHERE herb_id = $id";
                    $result = $conn->query($sql);

                    if ($result->num_rows > 0) {
                        while ($recipe = $result->fetch_assoc()) {
                            echo "<li><a href='recipeview.php?id=" . $recipe['id'] . "'>" . htmlspecialchars($recipe['title']) . "</a></li>";
                        }
                    } else {
                        echo "<p>No recipes available for this herb.</p>";
                    }
                ?>
            </ul>
        </div>
